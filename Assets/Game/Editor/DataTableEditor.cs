...
using System.IO;
using System.Text;
using Game.Core.DataTable;

public class DataTableEditor : Editor {

    [MenuItem("表格/生成表格读取文件")]
    static void CreateRead()
    {
        string savePath = "Game/Core/Config";
        foreach (Object o in Selection.GetFiltered(typeof(Object), SelectionMode.DeepAssets | SelectionMode.Assets))
        {
            ...
            sc.Write("\t\t\t\t\t\t\t Game.Core.Common.GameInstance.OnQuit += OnQuit;\n");
            sc.Write("\t\t\t\t\t\t\t}\n");
            sc.Write("\t\t\t\t\t return mDic;\n");
            sc.Write("\t\t\t\t}\n");

            sc.Write("\t }\n");
            sc.Write("\t static void OnQuit()\n\t {\n\t\t mDic = null;\n\t }\n");
